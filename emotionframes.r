...
newDir<-"C:/Users/Laura/Desktop/TGL data/summary/emotion/"
for (m in 13:35) {
	
	userID <- fileUserID[m]
	print(paste("Processing ", userID, sep=""))
	# locate correct directory
	rootDir<-paste("C:/Users/Laura/Desktop/TGL data/data/",userID,"/", sep="")
	setwd(rootDir)
	
	
	#create list of all touchFiles
	touchFiles<-list.files(pattern="*TouchLogfile*")
	if (length(touchFiles) > 0) {
		emotionFiles <- 0
		for (i in 1: length(touchFiles)) {
			if (getEmotion(touchFiles[i]) != "free" || getGesture(touchFiles[i]) == "free") {
				emotionFiles <- c(emotionFiles, touchFiles[i])
			}
		}
	}
	...
	if (length(emotionFiles) > 0) {		
		...
		for (j in 1:length(emotionFiles)) {
			...
			print(paste("Processing file: ", emotionFiles[j], sep=""))
			print(paste("which is: ", j, " of ", length(emotionFiles)))			
			...
			if (m == 1 || m == 2) { # because P01 and P02 have gaze data integrated in touch files
				...
				alldata <- read.delim(emotionFiles[j], header=TRUE)
				...
				alldata <- read.delim(emotionFiles[j], header=FALSE)
			...
			for (k in 1:nrow(thisTouchdata)) {
				print(paste("Processing row: ", k, " of ", nrow(thisTouchdata), " in file ", j, " of ", length(emotionFiles), sep=""))
			...
			write.csv(touchframevals, file=paste(substr(emotionFiles[j], 1, nchar(emotionFiles[j])-4), ".csv", sep=""), row.names=FALSE)
