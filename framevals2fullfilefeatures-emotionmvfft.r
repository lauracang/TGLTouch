...
windowFeatures <- c("max", "min", "mean", "median", "var", "totalvar", "auc", "Xmax", "Xmin", "Xmean", "Xmedian", "Xvar", "Xtotalvar", "Xauc", "Ymax", "Ymin", "Ymean", "Ymedian", "Yvar", "Ytotalvar", "Yauc", "Cmax", "Cmin", "Cmean", "Cmedian", "Cvar", "Ctotalvar", "Cauc", "Fmax", "Fmin", "Fmean", "Fmedian", "Fvar", "Ftotalvar", "Fauc", "time", "subject", "gender", "pet", "age",  "firstLang", "secondLang", "type")
...
for (j in 1:length(emotionFiles)) {
	...
	for (n in 1:nrow(thisTouchdata)) {
		if (timedata[n] >= 1000 && timedata[n] < lastWin){
	...
	winCalc <- c(findtuple(winfs), findtuple(winx), findtuple(winy), findtuple(winc), findtuple(winf), thisTime, thisSubName, thisGender, thisPetFreq, thisAge, thisLang1, thisLang2, thisEmotion)
...
write.csv(featureData, file="fullemotionmvfftfeaturefile.csv", row.names=FALSE)