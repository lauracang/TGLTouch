...
for (j in 1:length(emotionFiles)) {
	...
	while(winEnd < lastWin) {
		...
		winCalc <- c(findtuple(winfs), findtuple(winx), findtuple(winy), thisSubName, thisGender, thisPetFreq, thisAge, thisLang1, thisLang2, thisEmotion)
...
write.csv(featureData, file="emotionfeaturefile.csv", row.names=FALSE)